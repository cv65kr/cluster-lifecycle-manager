...
// Example:
//
// "10.0.0.0/8:4180-4180,4190-4190" would result in the ingress ranges:
// [{CIDR: "10.0.0.0/8", FromPort: 4180, ToPort: 4180},{CIDR: "0.0.0.0/0", FromPort: 4190, ToPort: 4190}]
func sgIngressRanges(ranges string) ([]SGIngressRange, error) {
	...
	for i, r := range rangesL {
		...
		} else if len(splitCIDR) == 1 {
			portRange = splitCIDR[0]
		} else {
			return nil, fmt.Errorf("invalid input for sgIngressRanges: %s", ranges)
		}
		_, cidrNet, err := net.ParseCIDR("0.0.0.0/0")
		if err != nil {
			return nil, err
		}
