...
{{- end -}}`
	...
	r, err := renderSingle(t, testTemplate, map[string]string{"sgIngressRanges": "10.0.0.0/8:0-100,300-400"})
	require.NoError(t, err)
...
`, r, "rendered template is incorrect")
	...
	_, err = renderSingle(t, `{{ sgIngressRanges "0-1-2 }}`, "")
	require.Error(t, err)

	_, err = renderSingle(t, `{{ sgIngressRanges "0-1,-2 }}`, "")
	require.Error(t, err)

	_, err = renderSingle(t, `{{ sgIngressRanges "30-20" }}`, "")
	require.Error(t, err)

	_, err = renderSingle(t, `{{ sgIngressRanges "0-200000" }}`, "")
	require.Error(t, err)
